	...
	function update( $new_instance, $old_instance )  {
		...
		return $instance;
		 ...
		 function widget( $args, $instance ) {
			...
			wp_enqueue_style( 'so-styles', SO_PLUGIN_URL . 'assets/css/odometer-theme-' . $odometer_styles[ $odometer_style ] );
			
			$odometer_fonts = array(
				'car' => '//fonts.googleapis.com/css?family=Arimo',
				'digital' => '//fonts.googleapis.com/css?family=Wallpoet',
				'slot' => '//fonts.googleapis.com/css?family=Rye',
				'train' => '//fonts.googleapis.com/css?family=Economica'
				);

			if ( array_key_exists( $odometer_style, $odometer_fonts ) )
				wp_enqueue_style( 'so-font', $odometer_fonts[ $odometer_style ] );
			wp_enqueue_script( 'so-odometer', SO_PLUGIN_URL . 'assets/js/odometer.js',	array( ) , false, true );			

			wp_enqueue_script( 'so-script', SO_PLUGIN_URL . 'assets/js/so-script.js', array( 'jquery', 'so-odometer' ), false, true );
			...
			$html .= $before_widget;
			if( $before_text ) {
				$html .= '<div>' . $before_text . '</div>';
			}
			
			$html .= '<div class="odometer" id="example1" style="font-size: '. $font_size .'px; margin: 0 auto;"> '
                . $start_num . ' </div>';
		   	
		  	
			if ( $after_text ) {
				$html .= '<div>'. $after_text . '</div>';
			}			
